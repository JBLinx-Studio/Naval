Shader "Azure Sky System/Static Clouds"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            // Clouds
            uniform float  _Azure_StaticCloudLayer1Rotation;
            uniform float  _Azure_StaticCloudLayer2Rotation;
            ...
            float4 fragment_program (Varyings Input) : SV_Target
            {
                ...
                // Optical depth
                float zenith = acos(saturate(dot(float3(0.0, 1.0, 0.0), viewDir))) * _Azure_SkyScale;
                ...
                miePhase = _Azure_MieG.x / pow(_Azure_MieG.y - _Azure_MieG.z * moonCosTheta, 1.5);
                ...
                float2 lowCloudUv = float2(-atan2(viewDir.z, viewDir.x), -acos(viewDir.y)) / float2(2.0 * PI, 0.5 * PI) + float2(-_Azure_StaticCloudLayer1Rotation, 0.0);
                float2 higCloudUv = float2(-atan2(viewDir.z, viewDir.x), -acos(viewDir.y)) / float2(2.0 * PI, 0.5 * PI) + float2(-_Azure_StaticCloudLayer2Rotation, 0.0);
